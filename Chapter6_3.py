...
# Code 6.21 — the following recreates the dataframe created with the
# sim_happiness function in the rethinking package
seed     = 1977
N_years  = 1000
max_age  = 65
N_births = 20
...
# Code 6.22 – keep only adults and rescale age to [0, 1]
d2 = d[d["age"] > 17].copy()
d2["A"] = (d2["age"] - 18) / (65 - 18)

# Code 6.23 — create the index variable for marriage status and fit the model using Laplace approximation
...
print(az.summary(idata_m69, var_names=["a", "bA", "sigma"], ci_prob=0.89, round_to=2, kind="stats"))

# Code 6.24 — approximating the relationship between age and happiness without accounting for marriage status
with pm.Model() as model_m610:
...
print(az.summary(idata_m610, var_names=["a", "bA", "sigma"], ci_prob=0.89, round_to=2, kind="stats"))


### 6.3.2 The haunted DAG ###

# Code 6.25 — simulate the data for the haunted DAG example
...
# Code 6.26 — draw random samples for the variables in the haunted DAG
...
# Code 6.27 — model with both P and G as predictors of C
with pm.Model() as model_m611:
    a = pm.Normal("a",    mu=0, sigma=1)
    b_PC = pm.Normal("b_PC", mu=0, sigma=1)
    b_GC = pm.Normal("b_GC", mu=0, sigma=1)
    sigma = pm.Exponential("sigma", lam=1)
    mu = a + b_PC * d["P"] + b_GC * d["G"]
    pm.Normal("C", mu=mu, sigma=sigma, observed=d["C"].to_numpy())
    idata_m611 = fit_laplace(draws=10_000)

print(az.summary(idata_m611, var_names=["a", "b_PC", "b_GC", "sigma"], ci_prob=0.89, round_to=2, kind="stats"))

# Code 6.28 — model with both P, G, and U as predictors of C
with pm.Model() as model_m612:
    a = pm.Normal("a",    mu=0, sigma=1)
    b_PC = pm.Normal("b_PC", mu=0, sigma=1)
    b_GC = pm.Normal("b_GC", mu=0, sigma=1)
    b_U = pm.Normal("b_U",  mu=0, sigma=1)
    sigma = pm.Exponential("sigma", lam=1)
    mu = a + b_PC * d["P"] + b_GC * d["G"] + b_U * d["U"]
    pm.Normal("C", mu=mu, sigma=sigma, observed=d["C"].to_numpy())
    idata_m612 = fit_laplace(draws=10_000)

print(az.summary(idata_m612, var_names=["a", "b_PC", "b_GC", "b_U", "sigma"], ci_prob=0.89, round_to=2, kind="stats"))
